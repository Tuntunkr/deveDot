...
function NodeSoftware({ projectList }) {
  const [nodesoftwareData, setNodeSoftwareData] = useState([]);

  useEffect(() => {
    console.log("projectList:", projectList);
    const newData = projectList?.filter((item) => {
      console.log("category:", item);
      return item?.categories?.includes("nodeSoftware");
    });
    console.log(newData);
    setNodeSoftwareData(newData);
  }, []);
